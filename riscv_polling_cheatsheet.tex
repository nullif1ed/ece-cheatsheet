...
\begin{lstlisting}
# PUSH values (stack grows DOWNWARDS)
...
\textbf{Rules:}
\begin{itemize}
\item Always adjust \texttt{sp} \textbf{first} when pushing
\item Always restore registers \textbf{before} adjusting \texttt{sp} when popping
\item Callee-saved: \texttt{s0--s11}, \texttt{ra}
\item Caller-saved: \texttt{t0--t6}, \texttt{a0--a7}
\end{itemize}

\section*{Interrupt Enabling}
\begin{lstlisting}
li t0, 0x80          
csrs mie, t0         # enable machine timer interrupt

li t0, 0x8           
csrs mstatus, t0     # set global MIE bit
\end{lstlisting}

\subsection*{Check Cause of Interrupt}
\begin{lstlisting}
csrr t0, mcause      # bit31 = interrupt flag, lower bits=cause
\end{lstlisting}
